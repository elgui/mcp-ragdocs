...
export class AddDocumentationEnhancedTool extends EnhancedBaseTool {
  ...
  async execute(args: any, callContext?: { progressToken?: string | number, requestId: string | number }): Promise<McpToolResponse> {
    ...
    if (!this.apiClient) {
      throw new McpError(ErrorCode.InternalError, 'API client is not initialized for AddDocumentationEnhancedTool');
    }
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i += batchSize) {
        ...
        const points = await Promise.all(
          batch.map(async (chunk) => {
            const embedding = await this.apiClient!.getEmbeddings(chunk.text); // Use non-null assertion after check
            return {
        ...
        if (!this.apiClient.qdrantClient) {
           throw new McpError(ErrorCode.InternalError, 'Qdrant client is not initialized for AddDocumentationEnhancedTool');
        }
        try {
      ...
      return this.formatTextResponse(
        `Successfully added documentation from ${args.url} (${chunks.length} chunks processed in ${Math.ceil(chunks.length / batchSize)} batches)`
      );
    } catch (error) {
      if (error instanceof McpError) {
        throw error;
      }
      return this.handleError(`Failed to add documentation: ${error}`);
    }
  }

  private async fetchAndProcessUrl(url: string): Promise<DocumentChunk[]> {
    if (!this.apiClient || !this.apiClient.browser) {
       throw new McpError(ErrorCode.InternalError, 'Browser is not initialized for AddDocumentationEnhancedTool');
    }
